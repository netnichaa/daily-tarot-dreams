...
import { CardTypeConfig } from '@/types/tarot';
...
export const TarotCard = ({ config, isRead, isSelected, onClick, index }: TarotCardProps) => {
  ...
  return (
    <button
      ...
      className={`
        ...
        ${isHovered && !isRead ? 'scale-105 -translate-y-2' : ''}
        ${isRead ? 'opacity-60 cursor-not-allowed grayscale-[30%]' : 'cursor-pointer'}
        group
      `}
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full p-3 text-center">
        ...
        <div className={`
          ...
          ${!isRead && isHovered ? 'scale-110 animate-float' : ''}
        `}>
          {config.icon}
        </div>
        ...
        <div className="absolute top-4 left-1/2 -translate-x-1/2">
          <Sparkles className={`
            ...
            ${!isRead && isHovered ? 'animate-glow-pulse' : ''}
          `} />
        </div>
        ...
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2">
          {isRead ? (
            <div className="flex items-center gap-1 text-xs text-muted-foreground">
              ...
              <span className="font-body">Read</span>
            </div>
          ) : (
            <div className={`
