...
import { CARD_TYPES, READING_COST, AD_REWARD, ProphecyType, CardTypeConfig, TarotCard as TarotCardType, TarotData } from '@/types/tarot';
import { Sparkles, Moon } from 'lucide-react';
import { toast } from 'sonner';
import tarotData from '@/data/tarotData.json';

const Index = () => {
  const { coins, isCardRead, spendCoins, earnCoins, markCardAsRead } = useGameState();
  
  const [selectedCardType, setSelectedCardType] = useState<CardTypeConfig | null>(null);
  ...
  return (
    <div className="relative min-h-screen min-h-[100dvh] overflow-hidden">
      ...
      {/* Main Content */}
      <main className="relative z-10 flex flex-col items-center justify-start min-h-screen min-h-[100dvh] px-4 pt-20 pb-8">
        {/* Header */}
        <header className="text-center mb-8 animate-fade-in-up">
          <div className="flex items-center justify-center gap-2 mb-2">
            <Sparkles className="w-5 h-5 text-primary animate-glow-pulse" />
            <h1 className="font-display text-2xl font-bold gold-text tracking-wide">
              Daily Tarot
            </h1>
            <Sparkles className="w-5 h-5 text-primary animate-glow-pulse" />
          </div>
        ...
        {/* Footer info */}
        <footer className="mt-auto pt-8 text-center animate-fade-in-up" style={{ animationDelay: '500ms' }}>
